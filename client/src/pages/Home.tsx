        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-6">
              Welcome back, <span className="text-red-400">{user?.firstName || user?.email || 'User'}</span>!
            </h1>
            <p className="text-lg lg:text-xl text-gray-200 mb-8 max-w-3xl mx-auto">
              Latest products और amazing deals - सब कुछ यहाँ मिलेगा best prices में!
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/shop">
                <Button className="bg-red-600 text-white px-8 py-3 hover:bg-red-700">
                  Browse Products
                </Button>
              </Link>
              <Link href="/deals">
                <Button className="bg-white text-red-600 px-8 py-3 hover:bg-gray-100">
                  View Deals
                </Button>
              </Link>
              {user?.isAdmin && (
                <Link href="/admin">
                  <Button variant="outline" className="border-white text-white px-8 py-3 hover:bg-white hover:text-red-600">
                    Admin Panel
                  </Button>
                </Link>
              )}
